...
1. **Componentes / repos** — how many components does the system need (frontend, backend,
   service, scripts, jobs), and what is each one's responsibility? What talks to what?
2. **Modelo de datos** — what entities exist, derived from what the PRD requires and what
   Design.md's screens reveal the user must see (the classic gap: a screen shows a discount, the
   model must have a discount field).
3. **Contratos de API** — shape of the interface between components (REST, RPC, GraphQL, direct
   function calls if it's a single deployable) and who owns the contract.
